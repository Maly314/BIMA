...
def parse_args():
    ...
    parser.add_argument("--variant", choices=("plain", "graph"), required=True)
...
def main():
    ...
    train_records = deterministic_subset(load_coco_records(root / "data" / "coco", "train"), args.train_limit, args.seed)
    val_records = deterministic_subset(load_coco_records(root / "data" / "coco", "val"), args.val_limit, args.seed + 1)
    ...
    model = KineResPose(args.graph_depth, args.decoder_channels, args.variant).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        progress = tqdm(loader, desc=f"kineres-{args.variant} epoch {epoch + 1}/{args.epochs}", unit="batch")
        ...
        report = {"epoch": epoch + 1, "seconds": time.perf_counter() - started, "train": running, "validation": metrics, "lr": scheduler.get_last_lr()[0]}
